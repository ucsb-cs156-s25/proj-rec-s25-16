...
describe("AdminRequestsPage tests", () => {
  ...
  test("renders without crashing on three requests", async () => {
    const threeRequestsFixture =
      recommendationRequestFixtures.threeRecommendations.map((req) => ({
        ...req,
        professor: req.professor || {
          fullName: "Default Prof",
          email: "prof@example.com",
        },
        requester: req.requester || {
          fullName: "Default Req",
    ...
    expect(
      await screen.findByTestId("RecommendationRequestTable-header-id"),
    ).toBeInTheDocument();
    expect(
      await screen.findByTestId(`RecommendationRequestTable-cell-row-0-col-id`),
    ).toHaveTextContent(threeRequestsFixture[0].id.toString());
  });
    ...
    await waitFor(() => {
      expect(
        axiosMock.history.get.filter(
          (req) => req.url === "/api/recommendationrequest/admin/all",
        ).length,
  ...
  test("Date formatting is applied in admin requests table", async () => {
    ...
    const idCell = await screen.findByTestId(
    ...
    expect(
      await within(tableElement).findByText("01:15:2023 02:05"),
    ).toBeInTheDocument();
    expect(
      within(tableElement).getByText("01:16:2023 06:30"),
    ).toBeInTheDocument();
    expect(
      within(tableElement).getByText("01:17:2023 01:15"),
    ).toBeInTheDocument();
    expect(
      within(tableElement).getByText("01:20:2023 09:45"),
    ).toBeInTheDocument();
  });
});
